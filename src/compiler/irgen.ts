import Cast from '../util/cast.ts';
import StringUtil from '../util/string-util.ts';
import BlockType from '../extension-support/block-type.ts';
import Variable from '../engine/variable.ts';
import log from '../util/log.ts';
import { IntermediateScript, IntermediateRepresentation } from './intermediate.ts';
import compatBlocks from './compat-blocks.ts';
 ...
 * @param {import('../engine/variable.ts')} varObj The Scratch Variable
 * @returns {*} A variable codegen object.
 */
const createVariableData = (scope: string, varObj: { id: any; name: any; isCloud: any; }) => ({
    scope,
    id: varObj.id,
    name: varObj.name,
 ...
 */
const parseProcedureCode = (variant: string) => variant.substring(1);

/**
 * @param {string} variant Variant generated by generateProcedureVariant()
 * @returns {boolean} true if warp enabled
 */
const parseIsWarp = (variant: string) => variant.charAt(0) === 'W';

export class ScriptTreeGenerator {
    private thread: {target: any; blockContainer: any};
    private target: any;
    private blocks: any;
    private runtime: any;
    private stage: any;
    ...
    variableCache: {};
    usesTimer: boolean;
    constructor (thread: { target: any; blockContainer: any; }) {
        /** @private */
        this.thread = thread;
        /** @private */
    ...
    getBlockInfo (fullOpcode: any) {
        ...
        const categoryInfo = this.runtime._blockInfo.find((ci: { id: any; }) => ci.id === category);
        if (!categoryInfo) {
            return null;
        }
        const blockInfo = categoryInfo.blocks.find((b: { info: { opcode: any; }; }) => b.info.opcode === opcode);
        if (!blockInfo) {
            return null;
        }
        return blockInfo;
     ...
     */
    descendInputOfBlock (parentBlock: { inputs: { [x: string]: any; }; opcode: any; }, inputName: string): Node {
     ...
     * @returns {Node} Compiled input node for this input.
     */
    descendInput (block: { opcode: any; fields: { [x: string]: { value: any; }; COLOUR?: any; NUM?: any; TEXT?: any; VALUE?: any; BROADCAST_OPTION?: any; NUMBER_NAME?: any; OPERATOR?: any; CURRENTMENU?: any; PROPERTY?: any; SOUND_MENU?: any; }; inputs: {}; }) {
        switch (block.opcode) {
        case 'colour_picker':
            return {
                kind: 'constant',
     ...
     */
    descendStackedBlock (block: { opcode: any; fields: { STOP_OPTION: { value: any; }; EFFECT: { value: string; }; FORWARD_BACKWARD: { value: string; }; FRONT_BACK: { value: string; }; STYLE: { value: any; }; }; mutation: { proccode: any; return: any; }; }) {
        switch (block.opcode) {
     ...
     */
    descendSubstack (parentBlock: { inputs: { [x: string]: any; }; }, substackName: string) {
     ...
     */
    descendVariable (block: { fields: { [x: string]: any; }; }, fieldName: string, type: string) {
     ...
     * @returns {*} A parsed variable object.
     */
    _descendVariable (id: string | number, name: any, type: any) {
    ...
    descendProcedure (block: { mutation: { proccode: any; }; inputs: { [x: string]: { block: any; }; }; id: any; }) {
        const procedureCode = block.mutation.proccode;
        const paramNamesIdsAndDefaults = this.blocks.getProcedureParamNamesIdsAndDefaults(procedureCode);
        if (paramNamesIdsAndDefaults === null) {
            return {
     ...
     */
    descendCompatLayer (block: { inputs: {}; fields: { [x: string]: { value: any; }; }; opcode: any; id: any; }) {
        this.script.yields = true;
    ...
    readTopBlockComment (commentId: string | number) {
    ...
    descendVisualReport (block: { next: any; }) {
     ...
     */
    walkHat (hatBlock: { next: any; opcode: any; id: any; }) {
...
export class IRGenerator {
    thread: { blockContainer: any; };
    blocks: any;
    proceduresToCompile: Map<any, any>;
    compilingProcedures: Map<any, any>;
    procedures: {};
    analyzedProcedures: never[];
    constructor (thread: { blockContainer: any; }) {
        this.thread = thread;
        this.blocks = thread.blockContainer;
     ...
     */
    generateScriptTree (generator: ScriptTreeGenerator, topBlockId: any) {
     ...
     */
    analyzeScript (script: { dependedProcedures: any; yields: boolean; }) {
